...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const highlightSection = document.querySelector(".highlight-section"); // если используешь подсветку
  ...
  function showSuggestions(value) {
    ...
    filtered.forEach(dish => {
      ...
      // клик по подсказке вставляет в input
  ...
  // ======= Highlight search matches на странице =======
  // Например, подсветка в разделе highlight-section
  const highlightInput = document.getElementById("highlightInput");
  const highlightBtn = document.getElementById("highlightBtn");

  if (highlightInput && highlightBtn && highlightSection) {
    highlightBtn.addEventListener("click", () => {
      const keyword = highlightInput.value.trim();
      if (!keyword) return;

      // удаляем старые подсветки
      highlightSection.innerHTML = highlightSection.textContent;

      // создаём регулярное выражение для поиска слова (чувствительность к регистру отключена)
      const regex = new RegExp(`(${keyword})`, "gi");

      // заменяем совпадения на span с классом highlight
      highlightSection.innerHTML = highlightSection.textContent.replace(
        regex,
